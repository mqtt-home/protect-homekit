...
func containsConflictingAnswers(req *Request, handle *serviceHandle) bool {
	...
	// Only look up our own A/AAAA records (which requires querying the
	// kernel for interface addresses) when the request actually contains
	// records for this service's hostname. Almost all mDNS traffic on a
	// network is from unrelated devices and is filtered out above.
	if len(reqAs) == 0 && len(reqAAAAs) == 0 {
		return false
	}
